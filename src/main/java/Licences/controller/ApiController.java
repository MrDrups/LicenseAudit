...
public class ApiController {
    ...
    public ResponseEntity<List<LicenseDTO>> getAllLicenses() {
        ...
        List<LicenseDTO> licenseDTOs = licenses.stream().map(LicenseMapper::toDTO).toList();
        return ResponseEntity.ok(licenseDTOs);
    }

    @GetMapping("/licenses/{id}")
    public ResponseEntity<LicenseDTO> getLicenseById(@PathVariable Long id) {
        return licenseRepository.findById(id).map(LicenseMapper::toDTO).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<LicenseDTO> createLicense(@RequestBody LicenseDTO licenseDTO) {
        ...
        license.setCompany(licenseDTO.getCompanyId() != null ? companyRepository.findById(licenseDTO.getCompanyId()).orElse(null) : null);
        license.setLicensePlan(licenseDTO.getLicensePlanId() != null ? licensePlanRepository.findById(licenseDTO.getLicensePlanId()).orElse(null) : null);
    ...
    public ResponseEntity<LicenseDTO> updateLicense(@PathVariable Long id, @RequestBody LicenseDTO licenseDTO) {
        ...
        }).map(LicenseMapper::toDTO).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<CompanyDTO> getCompanyById(@PathVariable Long id) {
        return companyRepository.findById(id).map(CompanyMapper::toDTO).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
    ...
    public ResponseEntity<List<LicensePlanDTO>> getAllLicensePlans() {
        ...
        List<LicensePlanDTO> licensePlanDTOs = licensePlans.stream().map(LicensePlanMapper::toDTO).toList();
    ...
    public ResponseEntity<LicensePlanDTO> getLicensePlanById(@PathVariable Long id) {
        return licensePlanRepository.findById(id).map(LicensePlanMapper::toDTO).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
